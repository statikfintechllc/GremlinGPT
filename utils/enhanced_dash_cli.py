...
class EnhancedDashboardCLI:
    ...
    def print_header(self, title: str):
        ...
        print(
            Colors.colorize(
                f"🧠 GremlinGPT Enhanced Dashboard - {title}", Colors.BOLD + Colors.CYAN
            )
        )
        print(Colors.colorize(f"Path: {self.current_path}", Colors.BLUE))
        print(
            Colors.colorize(
                f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", Colors.GREEN
            )
        )
        print("=" * width)
        print()
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            disk_usage = (
                subprocess.check_output(["df", "-h", str(self.project_root)], text=True)
                .strip()
                .split("\n")[1]
            )
    ...
    def display_main_menu(self):
        ...
        if "error" not in status:
            ...
            print(
                f"⚙️  Config: {Colors.colorize('Loaded' if status.get('config_loaded') else 'Not Found', Colors.GREEN if status.get('config_loaded') else Colors.RED)}"
            )
            print(
                f"📝 Logs: {Colors.colorize('Available' if status.get('logs_available') else 'Not Found', Colors.GREEN if status.get('logs_available') else Colors.RED)}"
            )
            ...
            try:
                ...
                if choice.lower() == "back":
                    ...
                    if file_path.exists() and file_path.is_file():
                        ...
                        print(
                            Colors.colorize(f"File not found: {filename}", Colors.RED)
                        )
    ...
    def display_file_list(self, items: List[FileItem]):
        ...
        print(
            f"{'#':<3} {'Name':<30} {'Type':<6} {'Size':<10} {'Modified':<20} {'Perm'}"
        )
        print("-" * 75)
    ...
    def view_file(self, file_path: Path):
        ...
        try:
            if file_path.suffix.lower() in [
                ...
                print(
                    Colors.colorize(
                        "Binary file - cannot display content", Colors.YELLOW
                    )
                )
    ...
    def configuration_manager(self):
        ...
        while True:
            ...
            print(
                Colors.colorize("📋 Configuration Sections:", Colors.BOLD + Colors.CYAN)
            )
    ...
    def log_monitor(self):
        ...
        while True:
            ...
            for i, (category, files) in enumerate(log_files.items(), 1):
                print(
                    f"{i}. {Colors.colorize(category, Colors.YELLOW)} ({len(files)} files)"
                )
    ...
    def browse_log_category(self, category: str, files: List[Path]):
        ...
        while True:
            ...
            for i, file in enumerate(files, 1):
                ...
                modified = (
                    datetime.fromtimestamp(file.stat().st_mtime).strftime("%m-%d %H:%M")
                    if file.exists()
                    else "N/A"
                )
                    ...
                    with open(log_file, "r", encoding="utf-8", errors="ignore") as f:
                        for line_num, line in enumerate(f, 1):
                            if search_term.lower() in line.lower():
                                ...
                                print(
                                    f"{Colors.colorize(log_file.name, Colors.CYAN)}:{line_num}: {line.strip()}"
                                )
    ...
    def system_control(self):
        ...
        while True:
            ...
            print(
                Colors.colorize("🎮 System Control Options:", Colors.BOLD + Colors.CYAN)
            )
            ...
            try:
                ...
                for script_path in script_paths:
                    if script_path.exists():
                        ...
                        print(
                            Colors.colorize(
                                f"❌ Script not found: {script_path}", Colors.RED
                            )
                        )
    ...
    def show_system_status(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["pgrep", "-f", "gremlin"], capture_output=True, text=True
            )
    ...
    def unified_system_manager(self):
        ...
        print(
            Colors.colorize(
                "🧠 This feature requires the unified system to be active.",
                Colors.YELLOW,
            )
        )
        ...
        print()
        print(
            "To activate the unified system, use 'System Control > Launch Unified System'"
        )
    ...
    def performance_monitor(self):
        ...
        print(
            Colors.colorize("📈 System Performance Metrics:", Colors.BOLD + Colors.CYAN)
        )
    ...
    def service_manager(self):
        ...
        print(
            Colors.colorize(
                "🔧 GremlinGPT Service Management:", Colors.BOLD + Colors.CYAN
            )
        )
        print()
        ...
        print(
            Colors.colorize(
                "⚠️ Service management is handled by the main system scripts.",
                Colors.YELLOW,
            )
        )
        ...
        try:
            while self.running:
                ...
                if choice == "1":
                    ...
                    print(
                        Colors.colorize(
                            "Invalid choice. Please select 1-9.", Colors.RED
                        )
                    )
                    time.sleep(1)
