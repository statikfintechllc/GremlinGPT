...
def safe_execute_with_fallback(
    primary_functions, fallback_functions, callback, *args, **kwargs
):
...
backend_loader = create_lazy_loader("backend.api_bridge", "send_to_frontend")
...
def requires_memory_functions(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if all(
            f is not None for f in [embed_text, package_embedding, inject_watermark]
        ):
...
def requires_orchestrator_functions(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if inject_task is not None:
            ...
            logger.warning(
                f"Skipping {func.__name__} - orchestrator functions not available"
            )
            return None
