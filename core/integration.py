...
class GremlinGPTUnifiedSystem:
    ...
    async def initialize_unified_system(self):
        ...
        try:
            ...
            logger.success(
                "[UNIFIED_SYSTEM] System initialization completed successfully"
            )
            log_event(
                "unified_system",
    ...
    async def _register_existing_modules(self):
        ...
        try:
            ...
            logger.info(
                f"[UNIFIED_SYSTEM] Registered {len(existing_modules)} existing modules"
            )
    ...
    async def _setup_fsm_integration(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "[UNIFIED_SYSTEM] Could not import execute_tool for integration"
                )
                ...
                if self._should_trigger_workflow(trigger, current_time):
                    ...
                    if self.async_loop and not self.async_loop.is_closed():
                        asyncio.run_coroutine_threadsafe(
                            self._execute_triggered_workflow(trigger), self.async_loop
                        )
                    else:
                        logger.warning(
                            "[UNIFIED_SYSTEM] Async loop not available for workflow execution"
                        )
    ...
    def _should_trigger_workflow(
        self, trigger: Dict[str, Any], current_time: datetime
    ) -> bool:
        ...
        if trigger_type == "periodic":
            ...
            time_diff = (
                current_time - datetime.fromisoformat(last_triggered)
            ).total_seconds() / 60
        ...
        # Task queue based trigger
            ...
            queue_length = sum(
                len(task_queue.task_queue[level]) for level in ["high", "normal", "low"]
            )
    ...
    async def _execute_triggered_workflow(self, trigger: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"[UNIFIED_SYSTEM] Executing triggered workflow: {workflow_type}"
            )

            if self.coordinator:
                result = await self.coordinator.execute_collaborative_workflow(
                    workflow_definition
                )
            else:
            ...
            logger.info(
                f"[UNIFIED_SYSTEM] Triggered workflow completed: {result.get('status', 'unknown')}"
            )
    ...
    async def _setup_workflow_triggers(self):
        ...
        try:
            ...
            logger.info(
                f"[UNIFIED_SYSTEM] Configured {len(self.workflow_triggers)} workflow triggers"
            )
    ...
    async def execute_unified_workflow(
        ...
        if self.coordinator:
            return await self.coordinator.execute_collaborative_workflow(
                workflow_definition
            )
        else:
    ...
    def inject_agent_task_to_fsm(self, agent_task: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"[UNIFIED_SYSTEM] Injected agent task to FSM: {fsm_task.get('type', 'unknown')}"
            )
            return True
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            coordinator_status = (
                await self.coordinator.get_coordination_status()
                if self.coordinator
                else {}
            )
            ...
            system_status = {
                ...
                "integration": {
                    ...
                    "async_thread_active": bool(
                        self.async_thread and self.async_thread.is_alive()
                    ),
        ...
        try:
            ...
            if self.fsm_integration_active and hasattr(self, "original_fsm_loop"):
                try:
                    ...
                    logger.error(
                        f"[UNIFIED_SYSTEM] Failed to restore original FSM loop: {e}"
                    )
...
if __name__ == "__main__":
    ...
    async def test_unified_system():
        ...
        result = await unified_system.execute_unified_workflow(
            ...
            {"symbols": ["AAPL", "GOOGL"], "analysis_depth": "standard"},
        )
