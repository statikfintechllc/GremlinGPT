...
class LearningAgent:
    ...
    async def monitor_performance(
        self, module_name: str, metrics: Dict[str, float]
    ) -> Dict[str, Any]:
        ...
        try:
            for metric_name, value in metrics.items():
                ...
                if metric_key not in self.performance_metrics:
                    ...
                    perf_metric.historical_values = [
                        (ts, val)
                        for ts, val in perf_metric.historical_values
                        if ts > cutoff_date
                    ]
                    ...
                    if old_value != 0:
                        ...
                        if (
                            abs(change_percent)
                            > self.config["improvement_threshold"] * 100
                        ):
                            opportunity = {
                                "metric": metric_name,
                            ...
                            analysis_result["learning_opportunities"].append(
                                opportunity
                            )

            # Generate recommendations
            recommendations = self._generate_performance_recommendations(
                module_name, analysis_result
            )
            ...
            log_event(
                ...
                "performance_monitored",
                {
                    ...
                    "opportunities_found": len(
                        analysis_result["learning_opportunities"]
                    ),
            ...
            logger.info(
                f"[LEARNING_AGENT] Monitored {module_name}: {len(analysis_result['learning_opportunities'])} opportunities identified"
            )

        except Exception as e:
            logger.error(
                f"[LEARNING_AGENT] Performance monitoring failed for {module_name}: {e}"
            )
    ...
    def _analyze_trend(
        ...
        values = [
            val for _, val in historical_values[-self.config["performance_window"] :]
        ]
    ...
    def _suggest_improvement_action(
        self, metric_name: str, change_percent: float, perf_metric: PerformanceMetric
    ) -> str:
    ...
    def _generate_performance_recommendations(
        self, module_name: str, analysis_result: Dict[str, Any]
    ) -> List[str]:
        ...
        if improvements:
            recommendations.append(
                f"Leverage {len(improvements)} positive performance changes for knowledge transfer"
            )

            # Specific recommendations for improvements
            for improvement in improvements[:3]:  # Top 3
                recommendations.append(
                    f"Study improvement in {improvement['metric']} (+{improvement['change']:.1f}%)"
                )

        if degradations:
            recommendations.append(
                f"Address {len(degradations)} performance degradations immediately"
            )

            # Specific recommendations for degradations
            for degradation in degradations[:3]:  # Top 3
                recommendations.append(
                    f"Fix degradation in {degradation['metric']} ({degradation['change']:.1f}%)"
                )
        ...
        if module_name == "trading_core":
            if any("accuracy" in opp["metric"] for opp in degradations):
                recommendations.append(
                    "Review and update trading algorithms with recent market data"
                )
            if any("profit" in opp["metric"] for opp in improvements):
                recommendations.append(
                    "Scale successful trading strategies across portfolio"
                )
            ...
            if any("speed" in opp["metric"] for opp in improvements):
                recommendations.append(
                    "Apply performance optimizations to other scrapers"
                )

        elif module_name == "nlp_engine":
            if any("accuracy" in opp["metric"] for opp in degradations):
                recommendations.append(
                    "Retrain NLP models with recent data and feedback"
                )

        return recommendations

    async def _update_learning_goals(
        self, module_name: str, analysis_result: Dict[str, Any]
    ):
    ...
    async def learn_from_feedback(
        ...
        try:
            ...
            knowledge_updates = self._update_knowledge_base(
                feedback_type, feedback_content, patterns, rating
            )
            learning_result["knowledge_updates"] = knowledge_updates

            # Adjust strategies based on feedback
            strategy_adjustments = await self._adjust_strategies_from_feedback(
            ...
            logger.info(
                f"[LEARNING_AGENT] Processed {feedback_type} feedback: {len(patterns)} patterns, {len(knowledge_updates)} updates"
            )
                ...
                patterns.append(
                    {
                        ...
                        "confidence": min(
                            1.0, positive_count / (positive_count + negative_count + 1)
                        ),
                        "details": {
    ...
    def _update_knowledge_base(
        ...
        kb_section["average_rating"] = (old_avg * old_count + rating) / kb_section[
            "feedback_count"
        ]
        ...
        for pattern in patterns:
            ...
            if pattern_key not in kb_section["common_patterns"]:
                kb_section["common_patterns"][pattern_key] = {
                    "count": 0,
                    "avg_confidence": 0.0,
                }
            ...
            pattern_info["avg_confidence"] = (
                old_pattern_conf * old_pattern_count + pattern["confidence"]
            ) / pattern_info["count"]
        ...
        if rating < 0.5:  # Negative feedback
            negative_patterns = [
                p for p in patterns if p.get("pattern") == "negative_feedback"
            ]
        ...
        # Identify success factors from positive feedback
            ...
            positive_patterns = [
                p for p in patterns if p.get("pattern") == "positive_feedback"
            ]
    ...
    async def _adjust_strategies_from_feedback(
        ...
        if rating > 0.7:  # Good feedback - exploit current strategies
            ...
            adjustments.append(
                f"Reduced exploration rate from {old_exploration:.3f} to {self.config['exploration_rate']:.3f}"
            )
            ...
            adjustments.append(
                f"Increased exploration rate from {old_exploration:.3f} to {self.config['exploration_rate']:.3f}"
            )
            ...
            for pattern in patterns:
                if pattern["type"] == "feature_feedback":
                    ...
                    if rating > 0.7 and feature not in strategy["success_patterns"]:
                        ...
                        adjustments.append(
                            f"Added {feature} to success patterns for {module}"
                        )

                    elif rating < 0.3 and feature not in strategy["avoid_patterns"]:
                        strategy["avoid_patterns"].append(feature)
                        adjustments.append(
                            f"Added {feature} to avoid patterns for {module}"
                        )

        return adjustments

    async def discover_patterns(
        self, data_sources: List[str], analysis_type: str = "correlation"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[LEARNING_AGENT] Pattern discovery complete: {len(discovery_result['patterns_found'])} patterns found"
            )
    ...
    async def _discover_correlations(self, data_sources: List[str]) -> Dict[str, Any]:
            ...
            if source in self.performance_metrics:
                for metric_name, metric in self.performance_metrics.items():
                    if metric_name.startswith(source):
                        values = [
                            val for _, val in metric.historical_values[-20:]
                        ]  # Last 20 values
        ...
        for i, metric1 in enumerate(metric_names):
            for j, metric2 in enumerate(metric_names[i + 1 :], i + 1):
                if (
                    len(metric_data[metric1]) == len(metric_data[metric2])
                    and len(metric_data[metric1]) >= 5
                ):
                    try:
    ...
    def _extract_patterns_from_correlations(
    ...
    def _generate_correlation_recommendation(
        self, corr_name: str, corr_data: Dict[str, Any]
    ) -> str:
    ...
    def _check_daily_pattern(
        ...
        variation = (
            statistics.stdev(avg_values) / statistics.mean(avg_values)
            if statistics.mean(avg_values) != 0
            else 0
        )
                    ...
                    if std_val > 0:
                        ...
                        for i, val in enumerate(values):
                            ...
                            if z_score > 2.5:  # More than 2.5 standard deviations
                                outliers.append(
                                    {"index": i, "value": val, "z_score": z_score}
                                )
    ...
    def _generate_pattern_recommendations(
        ...
        for pattern in patterns:
            ...
            if pattern_type == "correlation":
                ...
                recommendations.append(
                    f"Leverage {pattern_type.split('_')[1]} patterns in {metric} for predictive optimization"
                )
                ...
                if outlier_rate > 0.1:  # More than 10% outliers
                    recommendations.append(
                        f"Investigate and address high anomaly rate ({outlier_rate:.1%}) in {metric}"
                    )
                else:
                    recommendations.append(
                        f"Monitor outlier patterns in {metric} for early warning system"
                    )
    ...
    async def optimize_learning_parameters(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_history = (
                self.learning_history[-50:] if self.learning_history else []
            )
            ...
            if len(recent_history) >= 10:
                ...
                if effectiveness_score < 0.6:
                    ...
                    optimization_result["improvements_made"].append(
                        "Increased exploration rate for better learning"
                    )
                    ...
                    optimization_result["improvements_made"].append(
                        "Decreased exploration rate to exploit knowledge"
                    )
                else:
                ...
                avg_data_points = sum(
                    len(h.get("data", {}).get("patterns", [])) for h in recent_history
                ) / len(recent_history)
            ...
            logger.info(
                f"[LEARNING_AGENT] Parameter optimization complete. Score: {optimization_result['optimization_score']:.2f}"
            )
    ...
    def _calculate_learning_effectiveness(self, history: List[Dict[str, Any]]) -> float:
        ...
        pattern_discoveries = [
            h for h in history if h.get("type") == "pattern_discovery"
        ]
        if pattern_discoveries:
            avg_patterns = sum(
                len(pd.get("result", {}).get("patterns_found", []))
                for pd in pattern_discoveries
            ) / len(pattern_discoveries)
        ...
        feedback_processing = [
            h for h in history if h.get("type") == "feedback_learning"
        ]
        if feedback_processing:
            avg_updates = sum(
                len(fp.get("result", {}).get("knowledge_updates", []))
                for fp in feedback_processing
            ) / len(feedback_processing)
        ...
        performance_monitoring = [
            h for h in history if h.get("type") == "performance_monitoring"
        ]
        if performance_monitoring:
            avg_opportunities = sum(
                len(pm.get("result", {}).get("learning_opportunities", []))
                for pm in performance_monitoring
            ) / len(performance_monitoring)
            performance_score = min(
                1.0, avg_opportunities / 2
            )  # Normalize to 2 opportunities as max
    ...
    async def get_learning_summary(self) -> Dict[str, Any]:
        ...
        if recent_history:
            summary["learning_effectiveness"] = self._calculate_learning_effectiveness(
                recent_history
            )
...
if __name__ == "__main__":
    ...
    async def test_agent():
        ...
        test_metrics = {"accuracy": 0.85, "response_time": 1.2, "success_rate": 0.92}
