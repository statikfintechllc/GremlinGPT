...
class TradingStrategistAgent:
    ...
    async def generate_trading_signals(
        self, market_data: Dict[str, Any], symbols: Optional[List[str]] = None
    ) -> List[TradingSignal]:
        ...
        try:
            ...
            for symbol in symbols:
                if symbol not in market_data:
                    logger.warning(
                        f"[TRADING_STRATEGIST] No data available for {symbol}"
                    )
                    continue
                ...
                if (
                    signal
                    and signal.confidence >= self.config["signal_confidence_threshold"]
                ):
            ...
            signals_text = (
                f"Generated {len(signals)} trading signals for {len(symbols)} symbols"
            )
            ...
            logger.info(
                f"[TRADING_STRATEGIST] Generated {len(signals)} signals for {len(symbols)} symbols"
            )
            return signals
    ...
    async def _analyze_symbol(
        ...
        try:
            ...
            position_size = self._calculate_position_size(
                risk_assessment, current_price, stop_loss
            )
            ...
            signal = TradingSignal(
                ...
                supporting_indicators=self._get_supporting_indicators(
                    technical_indicators, signal_type
                ),
    ...
    def _calculate_technical_indicators(self, prices: List[float]) -> Dict[str, Any]:
        ...
        if "ema_12" in indicators and "ema_26" in indicators:
            ...
            macd_history = [
                indicators["macd"]
            ]  # Simplified - in real implementation, calculate for more periods
    ...
    def _determine_signal(self, indicators: Dict[str, Any]) -> Tuple[SignalType, float]:
        ...
        buy_confidence = sum(
            conf for i, conf in enumerate(confidences) if signals[i] == SignalType.BUY
        )
        sell_confidence = sum(
            conf for i, conf in enumerate(confidences) if signals[i] == SignalType.SELL
        )

        if buy_signals > sell_signals:
            final_signal = (
                SignalType.STRONG_BUY if buy_confidence > 2.0 else SignalType.BUY
            )
            final_confidence = min(1.0, buy_confidence / len(signals))
        elif sell_signals > buy_signals:
            final_signal = (
                SignalType.STRONG_SELL if sell_confidence > 2.0 else SignalType.SELL
            )
    ...
    def _assess_risk(
        self, symbol: str, prices: List[float], indicators: Dict[str, Any]
    ) -> RiskAssessment:
    ...
    def _calculate_price_target(
        self, current_price: float, signal_type: SignalType, indicators: Dict[str, Any]
    ) -> Optional[float]:
    ...
    async def optimize_portfolio(
        self, current_positions: Dict[str, Any], available_signals: List[TradingSignal]
    ) -> Dict[str, Any]:
        ...
        if portfolio_risk["concentration_risk"] > 0.3:
            ...
            if current_positions:
                largest_position = max(
                    current_positions.items(), key=lambda x: x[1].get("value", 0)
                )
    ...
    def _calculate_optimization_score(
        self, positions: Dict[str, Any], recommendations: List[Dict[str, Any]]
    ) -> float:
